...
| `use_route_set_transition_buffer` | OFF | gate-failed | Fixed sole sub-2pp non-risk route-leg suppression was a full-system no-op: WF/CPCV OOS deltas and route-set turnover delta were all zero. It remains false and must not be retuned or rerun. |
| `use_cd_trend_dedup` | OFF | gate-failed | One-shot `cd-trend-ownership-v1` was rejected: WF OOS delta `-0.049997`, WF PBO `0.5714`, and CPCV PBO `0.8000` failed. Keep the research path OFF; no retune or rerun. |
...
| Route-set transition buffer (`use_route_set_transition_buffer`, `route-set-transition-buffer-v1`) | Rejected | One-shot formal gate and supplemental check found a complete no-op: WF Δ `0.0`, CPCV Δ `0.0`, and route-set turnover `103.466858` in both baseline and candidate. | `building/reports/formal_gate/route-set-transition-buffer-v1/result.json`; `building/reports/formal_gate/route-set-transition-buffer-v1/COMPOSITE_DECISION.json`; `docs/history/2026-07-20_route_set_transition_experiment.md` | Keep OFF. Do not change 2pp or rerun; a different mechanism requires a new prior and manifest. |
| C/D trend ownership (`use_cd_trend_dedup`, `cd-trend-ownership-v1`) | Rejected | Full-sample CAGR improved by `0.1114pp`, but the pre-registered formal gate failed WF OOS delta (`-0.049997`), WF PBO (`0.5714`), and CPCV PBO (`0.8000`). MaxDD worsened `0.7994pp`; CPCV OOS delta and DSR passed. | `building/reports/formal_gate/cd-trend-ownership-v1/result.json`; `building/reports/formal_gate/cd-trend-ownership-v1/REPORT.md`; `docs/history/2026-07-20_cd_trend_ownership_experiment.md` | Keep OFF. Do not retune D1/D2 weights or rerun this ownership experiment; a different mechanism needs a new prior and manifest. |
