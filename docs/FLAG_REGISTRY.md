...
| Deployment baseline | Current comparator | Docs, validation provenance | `docs/BASELINE_CURRENT.md`; cache v4 `baseline.json` is `CURRENT_EXECUTION_EVIDENCE` at gate-code commit `07f438d`, `equity_timing=next_open`, 15.58% CAGR / -20.83% MaxDD / 1.064 Sharpe | Rebuild after any gate-code/config/history/soft-history provenance change; baseline alone authorizes no flip |
