...
| Deployment baseline | Current comparator | Docs, validation provenance | `docs/BASELINE_CURRENT.md`; cache v4 `baseline.json` is `CURRENT_EXECUTION_EVIDENCE` at commit `80c64b7`, `equity_timing=next_open`, 15.58% CAGR / -20.83% MaxDD / 1.064 Sharpe | Rebuild after any code/config/history/soft-history provenance change; baseline alone authorizes no flip |
