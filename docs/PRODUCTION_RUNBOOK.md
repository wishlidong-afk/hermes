...
- FRED PIT 取证：raw 证据保留 observations API 查询的 `realtime_start/realtime_end`、`fetched_at`、source URL 和 PIT 规则；这些 realtime 字段是查询 vintage，不是逐行首次发布日。canonical 仍用 `observation_date + 1 day`，除非未来单独建设并 gate ALFRED 首发 vintage 数据集。
