...
> 完成并人审后,T7 由你翻闸。我不会替你翻。

## 6. T5 已建 + 首次测量结果(`scripts/parity_harness.py`)

已建 parity harness(单体侧取自 golden 沙箱决策,包侧跑 `score_pipeline`,跑前备份/
跑后还原 signal journal,**不污染生产**)。6 个交易日 × 3 标的的首测:

| 字段 | 一致率 | 解读 |
|---|---:|---|
| **hard-valve ids** | **100% (18/18)** | ✅ 安全关键闸**已完全对齐**——M4 安全门已满足 |
| status | 44.4% (8/18) | 🔴 **根因**:包给 HOLD 而单体给 WATCH/REDUCE → 评分阈值/数值有差 |
| sell_pct | 72.2% (13/18) | 随 status 级联 |
| destination | 72.2% (13/18) | 随 status 级联(归一化 None/"-" 后,半数原"差异"是 sentinel 噪声) |
| routed | 72.2% (13/18) | 同上 |

**结论**:**硬阀门(安全)已 100% 对齐**;真正的 parity 缺口集中在**评分(status)**,
并级联到减仓/路由。故 **T2(评分对齐)是关键路径**,与本计划预判一致。

> 注意 caveat:V1 输入来自各自 loader(单体=冻结 fixtures,包=实盘 store),故部分 status
> 差异可能含数据差,不全是代码差。要彻底分离代码 vs 数据,需 T1(同输入桥)。下一步:
> **建 T1 同输入桥**(让包消费同一 `daily_raw_data` fixtures)→ 把 status 缺口归因到具体因子,
> 再做 T2 逐因子对齐。
