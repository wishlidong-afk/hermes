...
DEFAULT_OUT = REPO_ROOT / "docs" / "history" / "BASELINE_2026_06_11.md"
DEFAULT_CONTEXT = REPO_ROOT / "context.md"
LEGACY_RESEARCH_WARNING = (
    "STALE RESEARCH EVIDENCE: the stored flag/routing gate artifacts predate "
    "the formal IS-selection/OOS-PBO gate and no longer match the current code/data "
    "fingerprint. All performance and OOS-rank values below are historical diagnostics "
    "and cannot authorize a new flag or routing change."
)
...
def _read_gate_rows(relpath: str) -> list[dict[str, str]]:
    ...
    for line in text.splitlines():
        ...
        if len(cells) == 8:
            headers += ["median_oos_obj", "delta_vs_base", "bottom_half_rate", "dsr", "historical_gate"]
        elif len(cells) >= 11:
            headers += [
                "sharpe",
                "calmar",
...
def _historical_gate_result(row: dict[str, str]) -> str:
    text = row.get("historical_gate", row.get("authorization", ""))
    return text.replace("PBO≥.5", "bottom-half rate≥.5")
...
def build_doc() -> str:
    ...
    lines = [
        ...
        f"> **{LEGACY_RESEARCH_WARNING}**",
        "",
        "## Scope",
        "",
        "This file preserves the historical performance and validation baseline used by `docs/OPTIMIZATION_ROADMAP.md` T4. "
        "The current deployed routing overlay is DEFCON1 BOXX/DBMF/IAU plus DEFCON3 MSTR routed to BTC-USD, "
        "with F3/F4/F5/F6 live, and H-M2 buffer still OFF. The old forward-optimization reference is the "
        "`cot_nq` gate baseline row; it is now STALE and retained for historical comparison only. "
        ...
        f"| Routing-gate note (contains legacy PBO wording) | {routing['gate_note']} | config `_defcon3_note`; historical wording only, not current authorization evidence |",
        f"| Routing-gate artifacts | {routing_artifacts} | local worktree check |",
        f"| Historical forward-optimization reference (STALE) | {cot_baseline['full_cagr']} CAGR / {cot_baseline['full_maxdd']} MaxDD / {cot_baseline.get('sharpe', 'n/a')} Sharpe / {cot_baseline.get('calmar', 'n/a')} Calmar | `building/reports/flag_sweep/GATE_REPORT_cot_nq.md` baseline row |",
        f"| Legacy OOS bottom-half rate / DSR (diagnostic only) | {cot_baseline.get('bottom_half_rate', 'n/a')} / {cot_baseline.get('dsr', 'n/a')} | same; not formal PBO and not authorization evidence |",
        ...
        "### Flag gate",
        "",
        "| Variant | Full CAGR | Full MaxDD | Median OOS obj | Δ vs base | OOS bottom-half rate | DSR | Historical result |",
        "|---|---:|---:|---:|---:|---:|---:|---|",
    ]
    for row in gate_rows:
        lines.append(
            ...
            f"{row.get('bottom_half_rate', '')} | {row.get('dsr', '')} | {_historical_gate_result(row)} |"
        )
    ...
    lines += [
        ...
        "### COT NQ gate",
        "",
        "| Variant | Full CAGR | Full MaxDD | Sharpe | Calmar | Median OOS obj | Δ vs base | OOS bottom-half rate | DSR | Historical result |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---|",
    ]
    for row in cot_rows:
        lines.append(
            ...
            f"{row.get('delta_vs_base', '')} | {row.get('bottom_half_rate', '')} | {row.get('dsr', '')} | "
            f"{_historical_gate_result(row)} |"
        )
    ...
    lines += [
        ...
        f"- The historical forward-optimization reference is **{cot_baseline['full_cagr']} CAGR / {cot_baseline['full_maxdd']} MaxDD / {cot_baseline.get('sharpe', 'n/a')} Sharpe / {cot_baseline.get('calmar', 'n/a')} Calmar** from the `cot_nq` gate baseline row; it is not a current deployment baseline.",
...
def update_context(context_path: Path, doc: str) -> None:
    ...
    replacement = "\n".join([
        ...
        f"| 历史路由门控说明（含旧 PBO 文案） | {routing['gate_note']} | config `_defcon3_note`；不可作为当前授权证据 |",
        f"| routing-gate 本地产物 | {routing_artifacts} | 本地文件检查 |",
        f"| 历史含 combo 参照 CAGR（STALE） | {cot_baseline['full_cagr']} | `building/reports/flag_sweep/GATE_REPORT_cot_nq.md` baseline row |",
        f"| 历史含 combo 参照 Max Drawdown（STALE） | {cot_baseline['full_maxdd']} | `building/reports/flag_sweep/GATE_REPORT_cot_nq.md` baseline row |",
        f"| 历史含 combo 参照 Sharpe（STALE） | {cot_baseline.get('sharpe', 'n/a')} | `building/reports/flag_sweep/GATE_REPORT_cot_nq.md` baseline row |",
        f"| 历史含 combo 参照 Calmar（STALE） | {cot_baseline.get('calmar', 'n/a')} | `building/reports/flag_sweep/GATE_REPORT_cot_nq.md` baseline row |",
    ...
    pattern = re.compile(r"## 11\. 当前性能基线(?:与路由部署态)?(?:（2026-06-11）)?\n.*?\n---", re.S)
    new_text, count = pattern.subn(replacement + "\n\n---", text, count=1)
    if count != 1:
        raise RuntimeError("Could not locate context.md section 11")
