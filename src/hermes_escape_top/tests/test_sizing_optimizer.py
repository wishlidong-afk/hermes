...
def _make_risk_state(
    ...
    gross_scaler: float | None = None,
) -> RiskState:
    cov = np.eye(n) * (vol ** 2)
    gross = min(1.0, vol_budget / max(vol, 1e-9)) if gross_scaler is None else gross_scaler
    return RiskState(
        cov=cov,
        corr=np.eye(n),
...
class TestLiquidityCap(unittest.TestCase):
    def test_returns_reasonable_cap(self) -> None:
        cap = liquidity_cap(adv20_shares=1_000_000, price=50, netliq=5_000_000, cfg={"max_liquidation_days": 3, "participation_rate": 0.10})
        self.assertGreater(cap, 0.0)
        self.assertLessEqual(cap, 1.0)

    def test_zero_adv_returns_zero(self) -> None:
        cap = liquidity_cap(adv20_shares=0, price=50, netliq=5_000_000, cfg={})
...
class TestOptimizeTargets(unittest.TestCase):
    ...
    def test_risk_gross_scaler_shrinkage(self) -> None:
        verdicts = _make_verdicts(3, rule_weight=0.2)
        risk_full = _make_risk_state(vol=0.2, gross_scaler=1.0)
        risk_low = _make_risk_state(vol=0.2, gross_scaler=0.5)
        conf = _make_confidence(1.0)
        r_full = optimize_targets(verdicts, risk_full, conf, _base_cfg())
        r_low = optimize_targets(verdicts, risk_low, conf, _base_cfg())
        self.assertLess(sum(r_low.target_weights.values()), sum(r_full.target_weights.values()))
        self.assertIn("risk gross scaler applied", " ".join(r_low.notes))
        self.assertTrue(any(v == "RISK_GROSS" for v in r_low.binding_constraint.values()))

    def test_hold_sleeves_do_not_collapse_without_alpha_model(self) -> None:
        verdicts = _make_verdicts(3, rule_weight=0.2)
        risk = _make_risk_state(vol=0.7, gross_scaler=0.7)
        conf = _make_confidence(1.0)
        result = optimize_targets(verdicts, risk, conf, _base_cfg())
        self.assertGreater(sum(result.target_weights.values()), 0.05)
    ...
    def test_binding_constraints_labeled(self) -> None:
        ...
        for sym in result.binding_constraint:
            self.assertIn(
                ...
                ("R3_RULE", "CONFIDENCE", "RISK_GROSS", "VOL_BUDGET", "ZERO", "NONE"),
            )
