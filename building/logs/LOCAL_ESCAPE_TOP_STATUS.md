...
| 15 集成、dry-run、切换 | **IN-PROGRESS / FULL-EXACT-PASSED / REVIEW-GATED** | local only | shadow + WARN sensitivity + full/exact passed | Phase II 252 日 shadow、相关闸敏感性、2113 日 full-window sensitivity 完成；P9 已复核 `110/0.90`：CAGR 20.37%、MaxDD -24.32%、R3=0、4 个 exact/fast 抽查通过；live 开关保持关闭 | 2026-06-02 |
...
| **P4 整合地基** | **DONE / PHASE0-I-PIPELINE-LOCAL-SYNCED** | `core/contracts.py`；`core/confidence/spine.py`；`core/portfolio/risk_engine.py`；`core/portfolio/sizing_optimizer.py`；`core/factors/lab.py`；`core/features/context.py`；`core/backtest/harness.py`；`core/pipeline.py`；`core/reentry/tracker.py`；`core/audit/exporter.py`；`reports/P4_LOCAL_SNAPSHOT_SYNC_LOG.md` | **270 package tests OK + 11 golden tests OK**；远端 P4 source snapshots 已落地本地；修复 integration_config 路径、Python 3.9 日期、RiskEngine 下行相关/数值稳定、SizingOptimizer shrinkage、MarketContext 测试警告 | live 开关保持关闭 |
| **P5 Phase II Shadow 对照** | **DONE / DRY-RUN-PACKAGE-READY** | `scripts/phase2_shadow_compare.py`；`scripts/phase2_corr_sensitivity.py`；`scripts/phase2_full_backtest_sensitivity.py`；`reports/PhaseII_Shadow_Compare.md/json`；`reports/PhaseII_Corr_Sensitivity.md/json`；`reports/PhaseII_Full_Backtest_Sensitivity.md/json`；`reports/P5_PHASE2_EXTENDED_DIAGNOSTICS_LOG.md` | 252 日 shadow：rows=252、errors=0、R3 violations=0、max abs weight delta=0.1592；2113 日 full sensitivity：errors=0、scenario count=21、R3 violations=0；review candidate `threshold=110 / penalty=0.70`，CAGR 18.06%、MaxDD -22.47%、Sharpe 1.0115、fixed OOS below-median 0.3077；4 个 exact spot-check PASS | live 开关保持关闭 |
| **P6 Phase III dry-run comparator** | **DONE / HUMAN-GATE-READY** | `scripts/phase3_dry_run_compare.py`；`tests/test_phase3_dry_run_compare.py`；`reports/PhaseIII_Dry_Run_Comparator.md/json`；`reports/P6_PHASE3_DRY_RUN_COMPARATOR_LOG.md` | 252 日 daily comparator：rows=252、errors=0、R3 violations=0、PASS=128、WARN=124、BLOCK=0；avg old/new turnover 0.2244/0.2285；当前总测试数见 P7 | 需人工审阅 WARN 日期；live 开关保持关闭 |
| **P7 Phase III WARN review** | **DONE / WARN-REVIEW-PACK-DONE** | `scripts/phase3_warn_review.py`；`tests/test_phase3_warn_review.py`；`reports/PhaseIII_WARN_Review.md/json`；`reports/P7_PHASE3_WARN_REVIEW_LOG.md` | WARN=124；EXTREME_CORR=102；WARN forward candidate-old avg：1d -0.00%、5d -0.05%、10d -0.29%；当前总测试数见 P8 | readiness=REVIEW_REQUIRED；live 开关保持关闭 |
| **P8 Phase III WARN sensitivity** | **DONE / WARN-SENSITIVITY-DONE** | `scripts/phase3_warn_sensitivity.py`；`tests/test_phase3_warn_sensitivity.py`；`reports/PhaseIII_WARN_Sensitivity.md/json`；`reports/P8_PHASE3_WARN_SENSITIVITY_LOG.md` | 24 场景；保留惩罚候选 `110/0.90`：WARN 10d -0.13%、max turnover delta 0.2886、R3=0、BLOCK=0；270 package tests OK + 11 golden tests OK | P9 full/exact 已完成；live 开关保持关闭 |
| **P9 Phase III 110/0.90 full/exact** | **DONE / FULL-EXACT-PASSED / REVIEW-GATED** | `reports/PhaseII_Full_Backtest_Sensitivity_P9_110_090.md/json`；4 组 P9 exact/fast 抽查报告；`reports/P9_PHASE3_110_090_FULL_EXACT_LOG.md` | 2113 日 full-window：errors=0、R3=0、CAGR 20.37%、MaxDD -24.32%、Sharpe 1.0171、turnover 338.0594；2020H1/2022H1/2024H1/2026YTD exact/fast 抽查均 PASS | 需 updated migration acceptance pack；需人工接受约 1.85 pp 回撤放大；live 开关保持关闭 |
