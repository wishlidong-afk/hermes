...
def portfolio_cvar(
    ...
    R = returns_df.dropna().values
    n, k = R.shape
    w = np.asarray(weights).flatten()[:k]
    if n < 20 or k < 1:
        return 0.0
    port_ret = R @ w
...
def build_risk_state(
    ...
    syms = sorted(leg_returns.keys())
    legs_reported = [s for s in syms if len(leg_returns[s].dropna()) >= min_periods]
    ...
    for s in legs_reported:
        leg_vol[s] = har_rv_forecast(leg_returns[s], risk_cfg)

    # Correlation: EWMA -> Ledoit-Wolf shrinkage
    joint_df = pd.DataFrame({s: leg_returns[s] for s in legs_reported}).dropna()
    n_obs = len(joint_df)
    ...
    if n_obs < min_periods:
        ...
        corr = ledoit_wolf_shrink(raw_corr, n_obs)
        corr_regime = "NORMAL"
    ...
    if factor_returns:
        fb, be = book_factor_beta(
            {s: leg_returns[s] for s in legs_reported},
            factor_returns,
