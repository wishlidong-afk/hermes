...
class TestBuildRiskState(unittest.TestCase):
    ...
    def test_nonfinite_returns_are_sanitized(self) -> None:
        dirty = {s: r.copy() for s, r in self.leg_returns.items()}
        dirty["LEG_0"].iloc[10] = np.inf
        dirty["LEG_1"].iloc[20] = -np.inf
        dirty["LEG_2"].iloc[30] = np.nan
        state = build_risk_state(dirty, self.target_weights, None, self.cfg)
        self.assertTrue(np.isfinite(state.cov).all())
        self.assertTrue(np.isfinite(state.corr).all())
        self.assertTrue(math.isfinite(state.portfolio_vol))
        self.assertTrue(math.isfinite(state.cvar))
